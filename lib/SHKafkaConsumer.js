 ...
 * @param {SHKafkaConsumerOption=} consumerOption The option to create a new instance of `Kafka.ConsumerGroup`.
...
class SHKafkaConsumer extends EventEmitter {
    ...
    _init(topics,consumerOption) {
        ...
        consumer.on('error',function(err) {
            _this.emit(SHKafkaConsumer.EVENT_CONSUMER_ERROR,err);
        });
    }
 ...
 */
SHKafkaConsumer.EVENT_CLIENT_READY = 'eventClientReady';
/**
 * The event to notify that the client is error.
 */
SHKafkaConsumer.EVENT_CLIENT_ERROR = 'eventClientError';
/**
 * The event to notify that an error ocurred in consumer.
 */
SHKafkaConsumer.EVENT_CONSUMER_ERROR = 'eventConsumerError';

module.exports = SHKafkaConsumer;
